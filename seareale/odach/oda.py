...
class MultiScaleFlip(Base):
    ...
    def batch_augment(self, images):
        self.imsize = images.shape[2]
        return F.interpolate(images, scale_factor=self.imscale, recompute_scale_factor=True).flip(3)
    def deaugment_boxes(self, boxes):
...
class MultiScaleHFlip(Base):
    ...
    def batch_augment(self, images):
        self.imsize = images.shape[2]
        return F.interpolate(images, scale_factor=self.imscale, recompute_scale_factor=True).flip(2)
    def deaugment_boxes(self, boxes):
        boxes[:, [0,2]] = self.imsize*self.imscale - boxes[:, [2,0]]
        boxes = boxes/self.imscale
        return boxes
